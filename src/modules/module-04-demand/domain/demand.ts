export const DEMAND_SIGNAL_TYPES = [
  'problem', 'search', 'conversation', 'content', 'commercial_intent',
  'transaction', 'creator', 'trend',
] as const
export type DemandSignalType = (typeof DEMAND_SIGNAL_TYPES)[number]

export const DEMAND_SOURCE_TYPES = [
  'tiktok', 'tiktok_shop', 'search', 'social', 'marketplace',
  'creator_content', 'user_input', 'external_research',
] as const
...
export const DEMAND_STATUSES = ['DISCOVERED','OBSERVING','VALIDATING','CONFIRMED','OPPORTUNITY_READY'] as const
...
export function calculateDemandScore(input: DemandSignalInput): number {
  ...
  const tierWeight = {
    transaction: 1, commercial_intent: 0.95, conversation: 0.9,
    search: 0.8, problem: 0.8, creator: 0.75,
    content: 0.55, trend: 0.45,
  }[input.signalType]
  const confidenceWeight = { low: 0.55, medium: 0.75, high: 0.9, very_high: 1 }[input.confidence]
  return Math.round(signal * tierWeight * confidenceWeight * 100) / 100
}

export function classifyDemandStatus(input: DemandSignalInput, score: number): DemandStatus {
  if (score >= 80 && ['high', 'very_high'].includes(input.confidence)) return 'OPPORTUNITY_READY'
