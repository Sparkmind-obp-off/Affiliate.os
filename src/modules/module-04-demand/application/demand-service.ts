...
import { calculateDemandScore, classifyDemandStatus, normalizeCanonicalProblem, type DemandSignalInput } from '../domain/demand.js'
...
export function parseDemandInput(payload: unknown): DemandSignalInput {
  ...
  for (const field of requiredStrings) if (typeof p[field] !== 'string' || !p[field].trim()) throw invalid(field)
  ...
  const input = p as unknown as DemandSignalInput
  const { isDemandSignalType, isDemandSourceType, isDemandConfidence } = requireGuards()
  if (!isDemandSignalType(input.signalType)) throw invalid('signalType')
  if (!isDemandSourceType(input.sourceType)) throw invalid('sourceType')
  if (!isDemandConfidence(input.confidence)) throw invalid('confidence')
  return input
}

export async function createDemandSignal(payload: unknown, workspaceId: string, repository: DemandSignalRepository): Promise<DemandSignalRepository['create'] extends (...args: any) => infer R ? Awaited<R> : never> {
...
async function fingerprintFor(workspaceId: string, input: DemandSignalInput, canonicalProblem: string): Promise<string> {
  const raw = JSON.stringify([workspaceId, canonicalProblem, input.audience.trim().toLowerCase(), input.keyword?.trim().toLowerCase() ?? '', input.signalType, input.sourceType, input.sourceRef?.trim() ?? '', input.observedAt])
...
function invalid(field = '(body)'): AppError {
  return AppError.validation('Demand signal payload is invalid', { issues: [{ field, message: 'invalid or missing value' }] })
}
function isUniqueViolation(error: unknown): boolean {
  return typeof error === 'object' && error !== null && 'code' in error && (error as { code?: string }).code === '23505'
}
function requireGuards() {
  return require('../domain/demand.js') as typeof import('../domain/demand.js')
}
