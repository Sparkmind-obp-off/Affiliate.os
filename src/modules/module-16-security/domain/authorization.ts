...
export type AuthorizationReason = 'authorized' | 'invalid_context' | 'inactive_account' | 'inactive_workspace' | 'missing_or_inactive_membership' | 'invalid_role' | 'missing_permission' | 'cross_tenant_resource'
export interface TenantResource { workspaceId: string }
export interface AuthorizationRequest { context: ResolvedIdentityContext; permission: string; resource?: TenantResource }
export interface AuthorizationDecision { effect: AuthorizationEffect; reason: AuthorizationReason }
...
export function permissionsForRole(role: string): readonly Permission[] { return isAuthorizationRole(role) ? ROLE_PERMISSIONS[role] : [] }
export function isPermission(permission: string): permission is Permission { return (PERMISSIONS as readonly string[]).includes(permission) }
export function isAuthorizationRole(role: string): role is AuthorizationRole { return Object.prototype.hasOwnProperty.call(ROLE_PERMISSIONS, role) }
export function authorize(request: AuthorizationRequest): AuthorizationDecision {
  ...
  if (!account.id || !workspace.id || !membership.id || membership.accountId !== account.id || membership.workspaceId !== workspace.id || (membership.role === 'owner' && workspace.ownerAccountId !== account.id)) return deny('invalid_context')
  ...
  if (!isPermission(permission) || !granted.includes(permission)) return deny('missing_permission')
